...
async function fillForm() {
  ...
  // The first 30 are indices 0 to 29. The new 25 are from index 30 onwards.
  const startIdx = 30;
  const limit = Math.min(startIdx + 25, formResponses.length);

  console.log(
    `Starting to fill form for NEWLY created ${limit - startIdx} users (Rows ${startIdx + 1} to ${limit}) with a 70-second delay between them...`,
  );
  ...
  for (let i = startIdx; i < limit; i++) {
    ...
    if (i < limit - 1) {
      console.log(`Waiting 70 seconds before the next submission...`);
      await new Promise((r) => setTimeout(r, 70000));
    }
  }

  await browser.close();
  console.log(`\n✅ Finished submitting forms for the ${limit - startIdx} new users!`);
}
